...
def train(model, train_prefetcher, psnr_criterion, pixel_criterion, optimizer, epoch, scaler, writer) -> None:
    ...
    while batch_data is not None:
        ...
        lrx8 = batch_data["lrx8"].to(config.device, non_blocking=True)
        lrx4 = batch_data["lrx4"].to(config.device, non_blocking=True)
        lrx2 = batch_data["lrx2"].to(config.device, non_blocking=True)
        ...
        with amp.autocast():
            srx2, srx4, srx8 = model(lrx8)
            loss = pixel_criterion(srx2, lrx4) + pixel_criterion(srx4, lrx2) + pixel_criterion(srx8, hr)
        ...
        psnr = 10. * torch.log10(1. / psnr_criterion(srx8, hr))
        losses.update(loss.item(), lrx8.size(0))
        psnres.update(psnr.item(), lrx8.size(0))
