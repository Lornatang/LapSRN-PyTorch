...
def rotate(image: np.ndarray, angle: int, center=None, scale_factor: float = 1.0) -> np.ndarray:
    ...
    Args:
        image (np.ndarray): The input image for `OpenCV.imread`.
        angle (int): Specify the rotation angle.
    ...
    rotate_image = cv2.warpAffine(image, matrix, (image_width, image_height * 2))
    rotate_image_height, rotate_image_width = rotate_image.shape[:2]

    for width in range(0, rotate_image_width):
        if rotate_image[:, width].any():
            left = width
            break
    for width in range(rotate_image_width - 1, 0, -1):
        if rotate_image[:, width].any():
            right = width
            break
    for height in range(0, rotate_image_height):
        if rotate_image[height, :].any():
            up = height
            break
    for height in range(rotate_image_height - 1, 0, -1):
        if rotate_image[height, :].any():
            down = height
            break

    new_image_height = abs(down - up)
    new_image_width = abs(right - left)
    new_image = np.zeros([new_image_height, new_image_width, 3], np.uint8)

    for width in range(new_image_width):
        for height in range(new_image_height):
            new_image[height, width] = rotate_image[up + height, left + width]
...
def random_rotate(lr_image: np.ndarray, hr_image: np.ndarray, angles: list, center=None, scale_factor: float = 1.0) -> [np.ndarray, np.ndarray]:
    ...
    rotated_lr_image = rotate(lr_image, angle, center, scale_factor)
    rotated_hr_image = rotate(hr_image, angle, center, scale_factor)
