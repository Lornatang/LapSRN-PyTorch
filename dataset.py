...
class TrainValidImageDataset(Dataset):
    ...
    def __getitem__(self, batch_index: int) -> [torch.Tensor, torch.Tensor]:
        ...
        lrx8_y_tensor = self.lrx8_datasets[batch_index]
        lrx4_y_tensor = self.lrx4_datasets[batch_index]
        lrx2_y_tensor = self.lrx2_datasets[batch_index]
        hr_y_tensor = self.hr_datasets[batch_index]

        return {"lrx8": lrx8_y_tensor, "lrx4": lrx4_y_tensor, "lrx2": lrx2_y_tensor, "hr": hr_y_tensor}
    ...
    def read_image_to_memory(self):
        ...
        for image_file_name in progress_bar:
            ...
            lrx8_image = imgproc.imresize(hr_image, 1 / 8)
            lrx4_image = imgproc.imresize(hr_image, 1 / 4)
            lrx2_image = imgproc.imresize(hr_image, 1 / 2)

            # Only extract the image data of the Y channel
            lrx8_y_image = imgproc.bgr2ycbcr(lrx8_image, use_y_channel=True)
            lrx4_y_image = imgproc.bgr2ycbcr(lrx4_image, use_y_channel=True)
            lrx2_y_image = imgproc.bgr2ycbcr(lrx2_image, use_y_channel=True)
            hr_y_image = imgproc.bgr2ycbcr(hr_image, use_y_channel=True)

            # Convert image data into Tensor stream format (PyTorch).
            # Note: The range of input and output is between [0, 1]
            lrx8_y_tensor = imgproc.image2tensor(lrx8_y_image, range_norm=False, half=False)
            lrx4_y_tensor = imgproc.image2tensor(lrx4_y_image, range_norm=False, half=False)
            lrx2_y_tensor = imgproc.image2tensor(lrx2_y_image, range_norm=False, half=False)
            hr_y_tensor = imgproc.image2tensor(hr_y_image, range_norm=False, half=False)

            self.lrx8_datasets.append(lrx8_y_tensor)
            self.lrx4_datasets.append(lrx4_y_tensor)
            self.lrx2_datasets.append(lrx2_y_tensor)
